function [Siphon] = FindMinimalSiphon(CurrentProblem,Trans_Input,Trans_Output,Nodes_Output,Nodes_Input)
...
while ~isempty(P_tilde)
    p = P_tilde(end);
    G_tilde = red(CurrentProblem{1},setdiff(Siphon,p),Trans_Input,Trans_Output);
    CurrentProblem_tilde{1} = G_tilde;
    CurrentProblem_tilde{2} = Pin_tilde;
    CurrentProblem_tilde{3} = [];

    [Siphon_tilde,CurrentProblem_tilde] = FindSiphon(CurrentProblem_tilde,Nodes_Input,Nodes_Output,Trans_Input,Trans_Output);
