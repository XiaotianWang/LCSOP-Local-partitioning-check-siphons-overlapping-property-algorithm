function [Siphon,CurrentProblem_Red] = FindSiphon_DCSOP_L(CurrentProblem,Nodes_Input,Nodes_Output,Trans_Input,Trans_Output)
...
while isReducible
    ...
    if ~isempty(Pout)
        G = red_DCSOP_L(CurrentProblem_Red{1},setdiff(P,Pout),Trans_Input,Trans_Output);
        % CurrentProblem_Red = [G,Pin,[]];
        CurrentProblem_Red{1} = G;
        CurrentProblem_Red{2} = Pin;
        CurrentProblem_Red{3} = [];
    end
    [isReducible,CurrentProblem_Red] = Reduce_DCSOP_L(CurrentProblem_Red,Trans_Input,Trans_Output,Nodes_Input,Nodes_Output);
