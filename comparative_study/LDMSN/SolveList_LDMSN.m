function [SOP] = SolveList_LDMSN(SiphonSet_old,ProblemSet,Nodes_Input,Nodes_Output,Trans_Input,Trans_Output)
...
while ~isempty(ProblemSet)
    ...
    [Siphon,CurrentProblem] = FindSiphon_LDMSN(CurrentProblem,Nodes_Input,Nodes_Output,Trans_Input,Trans_Output);

    if ~isempty(Siphon)
        Pin = CurrentProblem{2};

        if ~isequal(Siphon,Pin)
            Siphon = FindMinimalSiphon_LDMSN(CurrentProblem,Trans_Input,Trans_Output,Nodes_Output,Nodes_Input);
        end
        ...
        ProblemSet = Partition_LDMSN(ProblemSet,Siphon);
    end
